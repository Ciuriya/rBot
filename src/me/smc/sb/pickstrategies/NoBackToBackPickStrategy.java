...
public class NoBackToBackPickStrategy implements PickStrategy{
	...
	public void handleMapSelect(Game game, String map, boolean select, String mod){
		...
		if(selected != null && select){
			LinkedList<PickedMap> reversed = new LinkedList<>(manager.getPicks());
			Collections.reverse(reversed);
				
			PickedMap lastPick = null;
			for(PickedMap picked : reversed)
				if(picked.getTeam().getTeamName().equals(game.getNextTeam().getTeam().getTeamName()) &&
				   !picked.isWarmup()){
					lastPick = picked;
					break;
				}
			...
			if(lastModPicked != 0 && lastModPicked == selected.getCategory()){
				game.getBanchoHandle().sendMessage("You cannot select a map within this mod category twice in a row!", false);
